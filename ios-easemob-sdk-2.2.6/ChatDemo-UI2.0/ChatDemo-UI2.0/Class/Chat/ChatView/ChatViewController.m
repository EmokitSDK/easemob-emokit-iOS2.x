...
#warning EmoKit Add
            NSString *messageKey = [NSString stringWithFormat:@"%@", [[[model.chatVoice.localPath lastPathComponent] lastPathComponent] stringByDeletingPathExtension]];
...
#warning EmoKit Add
                /*
                若不是当前聊天对象的聊天界面，也需要在此识别语音的表情，
                因为list的delegate已被remove，所以这段代码2.x版本需注释掉
                BOOL isCurChat = [message.conversationChatter isEqualToString:self.conversation.chatter];
                if (!isCurChat)
                    return;
                 */
                __weak typeof(self) weakSelf = self;
                BOOL isFile = [[NSFileManager defaultManager] fileExistsAtPath:fileBody.localPath];
                
                MessageModel *messageModel = [MessageModelManager modelWithMessage:message];
                NSString *messageKey = [NSString stringWithFormat:@"%@", [[messageModel.localPath lastPathComponent] stringByDeletingPathExtension]];
                NSString *emo = [EMKEmoManager shareInstance].emoMessageDict[messageKey];
                BOOL isEmo = emo.length > 0 ? YES : NO;

                if (isFile && !isEmo)
                {
                    [[EMKEmoManager shareInstance] emoWithAMRFilePath:fileBody.localPath messageModel:messageModel completionHandler:^(NSString *emo, BOOL finished) {
...
{
#warning EmoKit Add
    MessageModel *messageModel = [self.dataSource lastObject];
    if (messageModel.type == eMessageBodyType_Voice)
    {
        NSString *amrPath = messageModel.localPath;
...
#warning EmoKit Add
            [[EMKEmoManager shareInstance] deleteMessageEmoWithArray:self.dataSource];
...
#warning EmoKit Add
                                [[EMKEmoManager shareInstance] deleteMessageEmoWithArray:weakSelf.dataSource];

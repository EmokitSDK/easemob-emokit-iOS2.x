...
#warning EmoKit Add
    ...
    BOOL isCurChat = NO;
    if (chatVC) {
        isCurChat = [message.conversationChatter isEqualToString:chatVC.conversation.chatter];
    }
    ...
    if (!error) {
        MessageModel *messageModel = (MessageModel *)[MessageModelManager modelWithMessage:message];
        if (messageModel.type == eMessageBodyType_Voice)
        {
            id<IEMMessageBody> messageBody = [message.messageBodies firstObject];
            EMVoiceMessageBody *voiceBody = (EMVoiceMessageBody*)messageBody;
            NSLog(@"voiceBody.attachmentDownloadStatus *** %lu", voiceBody.attachmentDownloadStatus);
            if (voiceBody.attachmentDownloadStatus == EMAttachmentDownloadSuccessed)
            {
                BOOL isFile = [[NSFileManager defaultManager] fileExistsAtPath:messageModel.localPath];
                if (isFile)
                {
                    [[EMKEmoManager shareInstance] emoWithAMRFilePath:messageModel.localPath messageModel:messageModel completionHandler:^(NSString *emo, BOOL finished) {
